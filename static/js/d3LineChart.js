var updateTimeChart = function(timeMessages) {
	...
	var setupTimeMessages = function(d) {
		var dateString = d.fields.date.toString();
		...
		dataAll.push({'value' : d.fields.num, 'date' : date});
	};
	for (var i = 0; i < timeMessages.length; i++) {
		setupTimeMessages(timeMessages[i]);
	}
	firstDraw();
